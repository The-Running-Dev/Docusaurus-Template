...
async function buildServer() {
  ...
  await app.register(cors, { origin: CORS_ORIGIN === '*' ? true : CORS_ORIGIN });
...
buildServer()
  .then(async (app) => {
    ...
    console.log(`🚀 API Server running on http://localhost:${PORT}${configService.getBasePath()}`);
    console.log(`📖 API Documentation available at http://localhost:${PORT}${configService.getBasePath()}/docs`);
  })
  .catch((err) => {
